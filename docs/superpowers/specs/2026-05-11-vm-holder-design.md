...
| 层 | 触发 | 动作 |
|---|---|---|
| 内层 (VBS 自身) | `sleepProc.Status <> 0` | Exec 重新 spawn sleep infinity |
| 外层 (任务计划) | wscript.exe 进程崩溃 | `RestartOnFailure Count=∞ Interval=30s` 重新拉起整个 VBS |

如果 WSL 整体故障(比如 LxssManager 服务挂了)导致 sleep infinity 持续 spawn 失败,VBS 不会无限快速 spawn——内层在 Exec 失败时也走 `WScript.Sleep 300000`(下一轮探活时再试),避免 fork bomb。
...
... [INFO] vm holder attached (sleep infinity child started)
...
| 风险 | 概率 | 缓解 |
|---|---|---|
| `WshShell.Exec` 启动控制台子进程时窗口闪烁(老 Windows 版本) | 低(已在 v2 hide-probe-window spec 排除过) | 实测确认;如有问题,改用 `WshShell.Run cmd, 0, False` 加 PID 跟踪(复杂但可行) |
