...
# 注意: SDKMAN 5.18+ 的 native CLI 把表格输出到 stderr,所以必须 2>&1 合并而不是
# 2>/dev/null 屏蔽 — 否则会拿到空字符串.
SDK_JAVA_LIST="$(as_login_shell 'sdk list java 2>&1' || true)"
...
as_login_shell "
  set -e
  if ! sdk list java 2>&1 | grep -qE 'installed.*${JDK8_RESOLVED}'; then
    sdk install java ${JDK8_RESOLVED} <<<n
  else
    echo 'JDK ${JDK8_RESOLVED} 已安装'
  fi
  if ! sdk list java 2>&1 | grep -qE 'installed.*${JDK17_RESOLVED}'; then
